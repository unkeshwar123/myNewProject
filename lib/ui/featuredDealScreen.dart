...
import 'package:p_assignment/viewModel/featuredDealViewModel.dart';
import 'package:retrofit/retrofit.dart';


class FeaturedDealScreen extends StatefulWidget {
  @override
  _FeaturedDealScreenState createState() => _FeaturedDealScreenState();
}
...
class _FeaturedDealScreenState extends State<FeaturedDealScreen> {
  ...
  final _featuredDealViewModel = Get.put(FeaturedDealViewModel());
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels ==
      ...
      if (!_featuredDealViewModel.isLoading.value) {
        _featuredDealViewModel.fetchData();
      }
    }
  }
          ...
          () => Stack(children: [
            ...
            RefreshIndicator(
                child: Column(children: [
                  Expanded(
                      child: !_featuredDealViewModel.isLoading.value
                          ? ListView.builder(
                        controller: _scrollController,
                        itemCount: _featuredDealViewModel.deals.length,
                  ...
                  if (_featuredDealViewModel.bottomloadingisLoading.value)
                    Padding(
                ...
                onRefresh: () async {
                  _featuredDealViewModel.refresh();
                }),
        ...
        children: [
          Row(
            ...
            children: [
              CachedNetworkImage(
                ...
                height: 70,
                width: 70,
                imageUrl: _featuredDealViewModel.deals[index].image_medium ?? '',
                errorWidget: (context, url, error) {
                  final errorWidget = Image.asset('assets/paritycube_logo.jpeg',
                      fit: BoxFit.fill);
                  return Image.network(
                      _featuredDealViewModel.deals[index].image_medium,
                      fit: BoxFit.fill,
          ...
          Row(
            children: [
              ...
              Text(
                _featuredDealViewModel.deals[index].comments_count.toString(),
                style: TextStyle(fontSize: 14, color: Colors.black38),
              ),
              SizedBox(
                width: 8,
              ...
              Text(
                getDate(_featuredDealViewModel.deals[index].created_at_in_millis),
                style: TextStyle(fontSize: 14, color: Colors.black38),
              ),
              Spacer(),
              Text(

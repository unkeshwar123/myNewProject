...
import 'package:p_assignment/viewModel/popularDealViewModel.dart';
import 'package:retrofit/retrofit.dart';

class PopularDealScreen extends StatefulWidget {
  @override
  _PopularDealScreenState createState() => _PopularDealScreenState();
}

class _PopularDealScreenState extends State<PopularDealScreen> {
  final _popularDealViewModel = Get.put(PopularDealViewModel());
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels ==
      ...
      if (!_popularDealViewModel.isLoading.value) {
        _popularDealViewModel.fetchData();
      }
    }
  }
    ...
    return Obx(
      () =>  Stack(children: [
          ...
          child: Column(children: [
            Expanded(
                child: !_popularDealViewModel.isLoading.value
                    ? ListView.builder(
                        controller: _scrollController,
                        itemCount: _popularDealViewModel.deals.length,
            ...
            if (_popularDealViewModel.bottomloadingisLoading.value)
              Padding(
        ...
        children: [
          Row(
            ...
            children: [
              CachedNetworkImage(
                ...
                height: 70,
                width: 70,
                imageUrl: _popularDealViewModel.deals[index].image_medium ?? '',
                errorWidget: (context, url, error) {
                  final errorWidget = Image.asset('assets/paritycube_logo.jpeg',
                      fit: BoxFit.fill);
                  return Image.network(
                      _popularDealViewModel.deals[index].image_medium,
                      fit: BoxFit.fill,
          ...
          Row(
            children: [
              ...
              Text(
                _popularDealViewModel.deals[index].comments_count.toString(),
                style: TextStyle(fontSize: 14, color: Colors.black38),
              ),
              SizedBox(
                width: 8,
              ...
              Text(
                getDate(
                    _popularDealViewModel.deals[index].created_at_in_millis),
                style: TextStyle(fontSize: 14, color: Colors.black38),
              ),
              Spacer(),
              Text(
